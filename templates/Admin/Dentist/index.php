		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Dentist'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
...
<div class="row">
	...
	<div class="table-responsive">
		...
		?>
                <tr>
					<th>#</th>
                    <th><?= $this->Paginator->sort('dentist_fullname') ?></th>
                    <th><?= $this->Paginator->sort('dentist_gender') ?></th>
                    <th><?= $this->Paginator->sort('dentist_phone') ?></th>
                    <th><?= $this->Paginator->sort('dentist_email') ?></th>                    
					<th><?= $this->Paginator->sort('dentist_id','ID') ?></th>
                    <th><?= $this->Paginator->sort('status') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($dentist as $dentist): ?>
                <tr>
				<td><?php echo $counter++ ?></td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $dentist->dentist_id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $dentist->dentist_id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Dentist: "{0}"?', $dentist->dentist_id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				<div class="col-md-4">
				  <div class="stat_card card-1 bg-body-tertiary">
					<h3><?php echo $total_dentist; ?></h3>
					<p>Total Dentist</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_dentist_active; ?></h3>
					<p>Active Dentist</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_dentist_archived; ?></h3>
					<p>Archived Dentist</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Dentist (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Dentist by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Dentist(s)',
			data: [<?= json_encode($total_dentist_active); ?>, <?= json_encode($total_dentist_disabled); ?>, <?= json_encode($total_dentist_archived); ?>],
            backgroundColor: [
    ...
    <div class="card bg-body-tertiary border-0 shadow mb-4">
        <div class="card-body">
            ...
            <?php // This line has been updated with the full, explicit URL
            ...
            <fieldset>
                <?php
                    // Using the more reliable Form->text()
                    echo $this->Form->label('id', 'Dentist ID');
                    echo $this->Form->text('id', [
                        'class' => 'form-control form-control-sm mb-2'
                    ]);

                    echo $this->Form->label('name', 'Dentist Name');
            ...
            <div class="text-end mt-3">
                <?php
                    if ($this->request->getQuery('id') || $this->request->getQuery('name')) {
