 ...
 * @var iterable<\App\Model\Entity\Patient> $patient
 */
        ...
        <div class="dropdown mx-3 mt-2">
            ...
            <i class="fa-solid fa-bars text-primary"></i>
                ...
                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
                        <li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Patient'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
                        </div>
        </div>
</div>
</div>
...
<div class="row">
    ...
    ?>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('patient_fullname', 'Full Name') ?></th>
                    <th><?= $this->Paginator->sort('patient_ic', 'IC Number') ?></th>
                    <th><?= $this->Paginator->sort('patient_gender', 'Gender') ?></th>
                    <th><?= $this->Paginator->sort('patient_phone', 'Phone') ?></th>
                    <th><?= $this->Paginator->sort('patient_email', 'Email') ?></th>
                    <th><?= $this->Paginator->sort('patient_id', 'ID') ?></th>
            ...
            <tbody>
                ...
                <td><?php echo $counter++ ?></td>
                    ...
                    <td><?= h($patient_item->patient_fullname) ?></td>
                    <td><?= h($patient_item->patient_ic) ?></td>
                    <td><?= h($patient_item->patient_gender) ?></td>
                    <td><?= h($patient_item->patient_phone) ?></td>
                    <td><?= h($patient_item->patient_email) ?></td>
                    <td><?= $this->Number->format($patient_item->patient_id) ?></td>
					<td><?= h($patient_item->state) ?></td>
                    <td>
    				<?php if ($patient_item->status): ?>
                    ...
                    <td class="actions text-center">
                        <div class="btn-group shadow" role="group" aria-label="Basic example">
                            <?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $patient_item->patient_id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
                            <?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $patient_item->patient_id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
                            <?php $this->Form->setTemplates([
                                'confirmJs' => 'addToModal("{{formName}}"); return false;'
                            ]); ?>
                            <?= $this->Form->postLink(
                                __('<i class="fa-regular fa-trash-can"></i>'),
                                ['action' => 'delete', $patient_item->patient_id],
                                [
                                    'confirm' => __('Are you sure you want to delete Patient: "{0}"?', $patient_item->patient_fullname),
                                    'title' => __('Delete'),
            ...
            <div class="row pb-3">
                ...
                <div class="col-md-4">
                    <div class="stat_card card-2 bg-body-tertiary">
                        <h3><?php echo $total_patient_active; ?></h3>
                        <p>Active Patient</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="stat_card card-3 bg-body-tertiary">
                        <h3><?php echo $total_patient_archived; ?></h3>
                        <p>Archived Patient</p>
                    </div>
                </div>
            </div>
...
<div class="row">
    ...
    <div class="card bg-body-tertiary border-0 shadow mb-4">
        <div class="card-body">
            <div class="card-title mb-0">Patient (Monthly)</div>
...
const monthly = new Chart(ctx, {
    ...
    <div class="card bg-body-tertiary border-0 shadow mb-4">
        <div class="card-body">
        <div class="card-title mb-0">Patient by Status</div>
            <div class="tricolor_line mb-3"></div>
        <div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Patient(s)',
            data: [<?= json_encode($total_patient_active); ?>, <?= json_encode($total_patient_disabled); ?>, <?= json_encode($total_patient_archived); ?>],
            backgroundColor: [
    ...
    <div class="card bg-body-tertiary border-0 shadow mb-4">
        <div class="card-body">
            ...
            <?php echo $this->Form->create(null, [
                'type' => 'get', // Ensures form uses GET method
                'valueSources' => 'query',
            ...
            <fieldset>
                <?php
                    ...
                    // NEW: Search by State (Dropdown)
                    echo $this->Form->control('state', [
                        'required' => false,
                        'label' => 'State',
                        'class' => 'form-select form-select-sm mb-2',
                        'options' => $states, // Uses the list from the controller
                        'empty' => 'All States'
                    ]);
            ...
            <div class="text-end mt-3">
                <?php 
                    ...
                    echo $this->Html->link(__('Reset'), ['action' => 'index'], ['class' => 'btn btn-outline-warning btn-sm']);
                    echo '&nbsp;&nbsp;';
